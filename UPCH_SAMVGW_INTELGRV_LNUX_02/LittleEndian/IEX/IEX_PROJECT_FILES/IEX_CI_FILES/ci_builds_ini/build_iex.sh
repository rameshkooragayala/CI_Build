...
if [ "$PLATFORM" == "gateway" ]; then 
	NDS_BUILD_ROOT=$BUILD_OUT_ROOT/UPCH_SAMVGW_INTELGRV_LNUX_01/LittleEndian/epg/fs/NDS
	DRIVERS_ROOT=$BUILD_OUT_ROOT/UPCH_SAMVGW_INTELGRV_LNUX_01/LittleEndian/drivers	
	...
	$BUILD_OUT_ROOT/UPCH_SAMVGW_INTELGRV_LNUX_01/LittleEndian/host/FusionConfigImport/FusionConfigImport  CFG2BIN ./ *.[cC][fF][gG]
	cd -
	...
	./BuildTools/makeImage.pl -project horizon -platform UPCH_SAMVGW_INTELGRV_LNUX_01 -vobroot $VOB_ROOT -fast -debug -auto -imagetype initramfs $NDS_BUILD_ROOT $DRIVERS_ROOT
	if [ "$?" -ne "0" ]; then
	...
	NDS_BUILD_ROOT=$BUILD_OUT_ROOT/UPCH_SAMISTB_INTELGRV_LNUX_01/LittleEndian/epg/fs/NDS
	DRIVERS_ROOT=$BUILD_OUT_ROOT/UPCH_SAMISTB_INTELGRV_LNUX_01/LittleEndian/drivers	
	...
	cd $NDS_BUILD_ROOT/config/
	$BUILD_OUT_ROOT/UPCH_SAMISTB_INTELGRV_LNUX_01/LittleEndian/host/FusionConfigImport/FusionConfigImport  CFG2BIN ./ *.[cC][fF][gG]
	cd -
	...
	./BuildTools/makeImage.pl -project horizon_istb -platform UPCH_SAMISTB_INTELGRV_LNUX_01 -vobroot $VOB_ROOT -fast -debug -auto -imagetype initramfs $NDS_BUILD_ROOT $DRIVERS_ROOT
	if [ "$?" -ne "0" ]; then
		echo "**** makeImage.pl FAILED ****"
		exit 1
	fi
	...
	NDS_BUILD_ROOT=$BUILD_OUT_ROOT/UPCH_SAMIPC_INTELGRV_LNUX_01/LittleEndian/epg/fs/NDS
	DRIVERS_ROOT=$BUILD_OUT_ROOT/UPCH_SAMIPC_INTELGRV_LNUX_01/LittleEndian/drivers
	...
	./BuildTools/makeImage.pl -project horizon_ipc -platform UPCH_SAMIPC_INTELGRV_LNUX_01 -vobroot $VOB_ROOT -fast -debug -auto -imagetype initramfs $NDS_BUILD_ROOT $DRIVERS_ROOT
	if [ "$?" -ne "0" ]; then
		echo "**** makeImage.pl FAILED ****"
		exit 1
	fi
