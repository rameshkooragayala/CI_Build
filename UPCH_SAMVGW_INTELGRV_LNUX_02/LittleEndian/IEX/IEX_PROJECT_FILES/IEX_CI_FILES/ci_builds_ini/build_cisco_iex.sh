...
if [ "$PLATFORM" == "gateway" ]; then 
	...
	$BUILD_OUT_ROOT/UPCH_CISCOUGW1726_INTELGRV_LNUX_01/LittleEndian/host/FusionConfigImport/FusionConfigImport  CFG2BIN ./ *.[cC][fF][gG]
	cd -
    ...
    ./BuildTools/makeImage.pl -project horizon -platform UPCH_CISCOUGW1726_INTELGRV_LNUX_01 -vobroot $VOB_ROOT -fast -debug -manual $NDS_BUILD_ROOT $DRIVERS_ROOT
	if [ "$?" -ne "0" ]; then
