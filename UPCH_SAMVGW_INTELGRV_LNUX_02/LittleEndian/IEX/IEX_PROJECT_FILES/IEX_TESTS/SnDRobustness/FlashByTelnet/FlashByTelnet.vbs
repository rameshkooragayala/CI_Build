...
'****h* UPC robustness script/FlashByTelnet
'* TITLE
'*  ST_ScriptTemplate_TestOnLoop
TEST_NAME ="FlashByTelnet"
TEST_CODE = "FLASH_Telnet"
sScriptVersionNumber = "v2.1"

'* COMPATIBILITY
'* dll IEX.ElementaryActions.E2E.UPC_v2.11.3
'* EdIni   v2.06
'* UserLib v2.03
...
'* SYNOPSIS
'*  The goal of this test is to flash the CPEs by telnet with the onsite configuration (local network)

'* HISTORY
'* v2.0
'* Script adapted for the S&D Infrastructure
'* v2.1
'* Remove the snapshots before and after the call of the EA.HL_LaunchFlashCPEByTelnet. Now the snapshots are directly integrated in the EA
...
'* ACTION
'* Call [EA.HL_LaunchFlashCPEByTelnet] to flash the new binary version

Set Res = EA.HL_LaunchFlashCPEByTelnet
If Not Res.CommandSucceeded Then
        Call EA.AC_Traces ("WARNING"," EA.HL_LaunchFlashCPEByTelnet failed because " & Res.FailureReason)
        Exit Sub
End If
...
Sub Initialize()
   ...
   'BMP_Path = "C:\Program Files\IEX" & tempTestLoc & "FlashByTelnet" & "_" & BMPSet & "_" & IP & "_" & ServerNo
   'BMP_Set = BMPSET 'Already set in Test_Main
   TEST_NAME = "FlashByTelnet"
   IEX.BitmapSet = BMP_Set
   IEX.BitmapSetTable = BMP_Path  & "\FlashByTelnet.xml"

   IEX.LogHeader "Test: FlashByTelnet."
...
Sub Test_Main(byval Debug, ByVal CurrentTestSet, ByVal CurrentTest, ByVal CurrentRun)
    ...
    strScriptFolder = "C:\Program Files\IEX\" & tempScriptLoc & "_" & "FlashByTelnet" & BMPSet & "_" & IP & "_" & ServerNo
    Call createFolderStruct(strScriptFolder)
    strLogFileNameXML = strScriptFolder  & "\TD_" & "FlashByTelnet" & ".xml"
    strLogFileNameXSL = strScriptFolder  & "\TD_" & "FlashByTelnet" & ".xsl"
    ...
    If Not Debug Then
     ...
     If IEX.IsConnected Then
        ...
        BMP_Path = "C:\Program Files\IEX\" & tempTestLoc & "_" & "FlashByTelnet" & BMPSet & "_" & IP & "_" & ServerNo
        BMP_Set = BMPSET
