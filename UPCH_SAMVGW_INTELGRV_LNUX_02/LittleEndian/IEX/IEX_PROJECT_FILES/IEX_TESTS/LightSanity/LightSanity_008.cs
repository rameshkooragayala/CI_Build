...
//LightSanity-008-RB-Review BufferPlayback & trickmodes
public class LightSanity_008 : _Test
{
    [ThreadStatic]
    static _Platform CL;
    ...
    private class Step3 : _Step
    {
        public override void Execute()
        {
            ...
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Failed to Verify Video is Present During Normal RB Play");
            }
    ...
    private class Step5 : _Step
    {
        public override void Execute()
        {
            ...
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Failed to Verify Video is Paused");
            }
            res = CL.IEX.MilestonesEPG.Navigate("TRICKMODE BAR");
            //res = CL.EA.PVR.SetTrickModeSpeed("RB", 1, false);
            if (!res.CommandSucceeded)
            ...
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Failed to Verify Video is Present During Normal RB Play on Another Channel");
            }
    ...
    private class Step6 : _Step
    {
        ...
        public override void Execute()
        {
            ...
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Failed to Verify Video is Present in Live Viewing After Skip in RB");
            }
