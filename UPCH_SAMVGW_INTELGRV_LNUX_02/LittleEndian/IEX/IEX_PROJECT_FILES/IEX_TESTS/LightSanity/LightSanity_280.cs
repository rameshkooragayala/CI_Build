...
//LightSanity-280-EPG-Thumbnail display
public class LightSanity_280 : _Test
{
    [ThreadStatic]
    static _Platform CL;
    ...
    private class Step2 : _Step
    {
        ...
        public override void Execute()
        {
            ...
            //Validate whether the thumbail recieved is default or not
            if (string.IsNullOrEmpty(thumbnail) || thumbnail.Equals(DefaultThumbnail) || thumbnail.Length.Equals(0))
            {
                FailStep(CL, res, "Thumbnail recieved is either null or default thumbnail " + thumbnail);

            }
            else
            {
                CL.IEX.LogComment("Thumbnail Recieved " + thumbnail);
            }
