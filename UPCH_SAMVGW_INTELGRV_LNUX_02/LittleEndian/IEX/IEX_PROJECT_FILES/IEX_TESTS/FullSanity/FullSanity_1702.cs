...
//FullSanity-1702-SUBT-TELETEXT subtitles change track
public class FullSanity_1702 : _Test
{
    [ThreadStatic]
    static _Platform CL;
    ...
    private class Step1 : _Step
    {
        public override void Execute()
        {
            ...
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Failed to Navigate to Portuguese Subtitles");
            }

            CL.IEX.Wait(15);

            //Change subtitles on current Event
            /*res = CL.EA.SubtitlesLanguageChange(EnumLanguage.Romanian);
            if (!res.CommandSucceeded)
            {
