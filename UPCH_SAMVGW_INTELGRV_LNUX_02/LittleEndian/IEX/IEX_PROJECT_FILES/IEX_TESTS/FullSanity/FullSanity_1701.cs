...
//FullSanity-1701-SUBT-DVB subtitles change track
public class FullSanity_1701 : _Test
{
    [ThreadStatic]
    static _Platform CL;
    ...
    private class PreCondition : _Step
    {
        public override void Execute()
        {
            ...
            CL.IEX.LogComment("Retrieved Value From ini File: Multiple_DVB_Subtitles_1 = " + Multiple_DVB_Subtitles_1);
    ...
    private class Step1 : _Step
    {
        public override void Execute()
        {
            ...
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Failed to Navigate to HEBREW Subtitles");
            }

            CL.IEX.Wait(15);

            //Change subtitles on current Event
            /*res = CL.EA.SubtitlesLanguageChange(EnumLanguage.Nederlands );
            if (!res.CommandSucceeded)
            {
                FailStep(CL,res, "Failed to Navigate to NEDERLANDS Subtitles");
            }*/
    ...
    private class Step2 : _Step
    {
        public override void Execute()
        {
            ...
            //Change subtitles settings to on - temmporary step !
            res = CL.IEX.MilestonesEPG.NavigateByName("STATE:AV SETTING SUBTITLES ON");
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Failed to Change Default Setting of Subtitles to ON");
            }
            ...
            if (subLang.Equals("OFF"))
            {
                FailStep(CL, "Failed to Activate the Subtitles (The Hightlighted Option is OFF)");
            }
