...
Namespace EAImplementation
    ...
    ''' </summary>
    Public Class VerifyEventInPlanner
        ...
        Protected Overrides Sub Execute()
            ...
            IsEmpty = EPG.FutureRecordings.isEmpty()
    
            If Me._SupposedToFindEvent And IsEmpty = False Then
                EPG.FutureRecordings.FindEvent(EventName, _Date, _StartTime, _EndTime)
            ElseIf Me._SupposedToFindEvent = False And IsEmpty Then
                Exit Sub
            ElseIf Me._SupposedToFindEvent And IsEmpty Then
            ExceptionUtils.ThrowEx(New EAException(ExitCodes.FindEventFailure, "Planner Is Empty"))

            ElseIf Me._SupposedToFindEvent = False And IsEmpty = False Then

                Try
                    EPG.Utils.StartHideFailures("Verifing Event : " + EventName + " Is Not On Planner")
                    EPG.FutureRecordings.FindEvent(EventName, _Date, _StartTime, _EndTime)
                ...
                If _StartTime <> "" OrElse _EndTime <> "" OrElse _Date <> "" Then
                    ExceptionUtils.ThrowEx(New EAException(ExitCodes.FindEventFailure, "Failed To Not Find Event : Event " + EventName + " With Start Time - " + _StartTime + " End Time - " + _EndTime + " Date - " + Me._Date + " Is On Planner"))
                Else
                    ExceptionUtils.ThrowEx(New EAException(ExitCodes.FindEventFailure, "Failed To Not Find Event : Event " + EventName + " Is On Planner"))
                End If
            End If
        End Sub
