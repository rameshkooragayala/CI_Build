...
Namespace EAImplementation
    Public Class MountTelnetStb
        ...
        Private _IsFactoryReset As Boolean
        ...
        Private TelnetLogFileName As String = ""
        Private UsePowerCycle As Boolean
        Private CheckForVideoAfterMount As Boolean = True
        Private CheckLegalDisclaimer As Boolean = False
        Private CheckLanguageSelection As Boolean = True
        ...
        Sub New(ByVal IsGw As Boolean, ByVal IsFormat As Boolean, ByVal DoReboot As Boolean, ByVal IsFactoryReset As Boolean, ByVal Retries As Integer, ByVal IsLastDelivery As Boolean, ByVal WakeUp As Boolean, ByVal IsReturnToLive As Boolean, ByVal m As IEX.ElementaryActions.Functionality.Manager)
            _manager = m
            _IsGw = IsGw
            _IsFormat = IsFormat
            _DoReboot = DoReboot
            _IsFactoryReset = IsFactoryReset
            _Retries = Retries
        ...
        Protected Overrides Sub Execute()
            ...
            End If
        End Sub
        Private Function MountTelnetUPCGw()
            ...
            Try
                CheckLegalDisclaimer = CBool(EPG.Utils.GetValueFromEnvironment("CheckLegalDisclaimer"))
            Catch ex As Exception
                CheckLegalDisclaimer = True
            End Try

            Try
                CheckLanguageSelection = CBool(EPG.Utils.GetValueFromEnvironment("CheckLanguageSelection"))
            Catch ex As Exception
            ...
            TelnetLogFileName = EPG.Mount.GetLogName(IsSerial:=False)
        ...
        Private Function MountGW(ByVal LoopNum As Integer, ByRef Msg As String) As Boolean
            ...
            EPG.Mount.BeginLogging(IsSerial:=False, logFileName:=TelnetLogFileName, loopNum:=LoopNum)

            EPG.Utils.LogCommentInfo("Trying To Mount STB Try : " + LoopNum.ToString + " Out Of " + _Retries.ToString)

            If _DoReboot Then
                'REBOOT
                EPG.Utils.LogCommentInfo("Deleting coredump if available")
                Dim changedirectory = "cd /corefile"
                Res = _iex.Debug.Write(changedirectory & vbCrLf, IEXGateway.DebugDevice.Serial)
                If Not Res.CommandSucceeded Then
                    ExceptionUtils.ThrowEx(New IEXException(Res))
                    EPG.Utils.LogCommentInfo("Failed to reach corefile directory " + changedirectory)
                Else
                    EPG.Utils.LogCommentInfo("Successfully reached " + changedirectory)

                    Dim rmcommand = "rm -rf core.*"
                    Res = _iex.Debug.Write(rmcommand & vbCrLf, IEXGateway.DebugDevice.Serial)
                    If Not Res.CommandSucceeded Then
                        ExceptionUtils.ThrowEx(New IEXException(Res))
                        EPG.Utils.LogCommentInfo("Failed to Delete core file")
                    Else
                        EPG.Utils.LogCommentInfo("Successfully Deleted the core file")
                        EPG.Utils.LogCommentInfo("Rebooting the Stb")
                        Res = _iex.Debug.Write("reboot" & vbCrLf, IEXGateway.DebugDevice.Serial)
                        If Not Res.CommandSucceeded Then
                            ExceptionUtils.ThrowEx(New IEXException(Res))
                            EPG.Utils.LogCommentInfo("Failed to reboot stb after deleting the core file")
                        End If
                        _iex.Wait(100)
                    End If
                End If
            ...
            If WaitForPrompt And Not _isNFS Then
            ...
            If ImageType <> "production" Then
                If Not _manager.TelnetLogIn(LoginToGw:=_IsGw) Then
                    EPG.Utils.LogCommentFail("Failed To Do Telnet Login")
                    Msg = "Failed To Do Telnet Login"
                    Return False
                End If

                'SENDS MOUNT COMMAND TO STB
                If Not EPG.Mount.SendMountCommand(IsSerial:=False, mountCommand:=MountCommand & vbCrLf) Then
            ...
            If _IsFormat Or _IsFactoryReset Then
                'EPG TEXT
                Try
        ...
        End Function
        Private Function MountTelnetCOGECOGw()
            Dim Passed As Boolean = True
        ...
        Private Function Mount(ByVal LoopNum As Integer, ByRef Msg As String) As Boolean
            ...
            If WaitForPrompt Then
                ...
                'SENDS MOUNT COMMAND TO STB
                If IsFormat Then
                    If Not EPG.Mount.SendMountCommand(IsSerial:=False, mountCommand:=MountCommand & vbCrLf, IsFormat:=True) Then
                        EPG.Utils.LogCommentFail("Failed To Send Mount Command : " + MountCommand + " To The STB")
                        Msg = "Failed To Send Mount Command : " + MountCommand + " To The STB"
                        Return False
                    End If
