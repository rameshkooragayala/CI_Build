...
public class FT172_0007 : _Test
{
    ...
    private class PreCondition : _Step
    {
        public override void Execute()
        {
            ...
            if (serviceList.Count < (NB_FAVORITES + 1))
            {
                FailStep(CL, "Failed to get " + (NB_FAVORITES + 1) + " clear services from Content.xml");
            }
            expFavorites = new List<Favourite>();
            string _favoriteChannelList = "";
            string _favoriteChannelNumList = "";
            for (int i = 0; i < NB_FAVORITES; i++)
            {
                expFavorites.Add(new Favourite() { channelName = serviceList[i].Name, channelNumber = i + 1 });
                _favoriteChannelList += serviceList[i].Name + ",";
                _favoriteChannelNumList += serviceList[i].LCN + ",";
            }
            _favoriteChannelList = _favoriteChannelList.TrimEnd(',');
            _favoriteChannelNumList = _favoriteChannelNumList.TrimEnd(',');
            
            // Fetch a non-favorite channel            
            nonFavChannel = serviceList[NB_FAVORITES];
            highestChannelNumber = CL.EA.GetValueFromINI(EnumINIFile.Test, "TEST PARAMS", "Highest_Service_Number");
    ...
    private class Step2 : _Step
    {
        public override void Execute()
        {
            ...
            CL.EA.UI.Utils.SendChannelAsIRSequence("1");
            CL.IEX.Wait(10);
    ...
    private class Step3 : _Step
    {
        public override void Execute()
        {
            ...
            CL.EA.UI.Utils.SendChannelAsIRSequence("4");
            CL.IEX.Wait(10);
