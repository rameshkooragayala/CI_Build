/// <summary>
///  Script Name : ON_PeriodicMP.
///  Test Name   : Periodic MP
///  TEST ID     : NA
///  QC Version  : NA
...
public class ON_PeriodicMP : _Test
{
    [ThreadStatic]
    ...
    {
        this.AddStep(new PreCondition(), "Precondition: Do some Recordings and wait for the box to go to Standy By");
        this.AddStep(new Step1(), "Step 1: Wait for the Recordings to complete and after the recording are completed wait for the Periodic Maintenance Phase after 22 hours");
    ...
    private class PreCondition : _Step
    {
        public override void Execute()
        {
            ...
            res = CL.EA.PVR.RecordManualFromPlanner("TIME_BASED", Convert.ToInt32(recordableService.LCN), DaysDelay: -1, MinutesDelayUntilBegining: 480, DurationInMin: 60, VerifyBookingInPCAT: false);
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Failed to record manual from planner on service " + recordableService.LCN);
            }
            //2nd Manual Recording
            res = CL.EA.PVR.RecordManualFromPlanner("TIME_BASED1", Convert.ToInt32(recordableService1.LCN), DaysDelay: -1, MinutesDelayUntilBegining: 480, DurationInMin: 60, VerifyBookingInPCAT: false);
            if (!res.CommandSucceeded)
            {
            ...
            res = CL.IEX.Wait(209 * 60);
            if (!res.CommandSucceeded)
            {
                FailStep(CL, "Failed to wait until the Notification");
            }
    ...
    private class Step1 : _Step
    {
        public override void Execute()
        {
            ...
            //After more then one hour wait inactivity before doing Maintenanace phase the box will do Reboot and will come to shell prompt
            string ActualLine = "";
            string timeStamp = "";
            res = CL.IEX.Debug.BeginWaitForMessage(prompt, 0, 15 * 60, debugDevice: IEXGateway.DebugDevice.Serial);
            if (!res.CommandSucceeded)
            {
            ...
            //Commenting as the box will reboot before doing the Maintenance Phase if there is inactivity
            ...
            res = CL.IEX.Debug.BeginWaitForMessage(prompt, 0, 500 * 60, debugDevice: IEXGateway.DebugDevice.Serial);
            if (!res.CommandSucceeded)
            {
                LogCommentFailure(CL, "Failed to start the begin debug message for shell prompt");
            }
            ...
            res = CL.IEX.Wait((22 * 60 * 60) - (Convert.ToInt32(sw.Elapsed.TotalSeconds)+1200));
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Failed to Wait");
            }
            //Waiting for the Periodic Maintenance to occur which will bring the Box to shell Prompt
            res = CL.IEX.Debug.BeginWaitForMessage(prompt, 0, 260 * 60, debugDevice: IEXGateway.DebugDevice.Serial);
            if (!res.CommandSucceeded)
            {
                LogCommentFailure(CL, "Failed to start the begin debug message for shell prompt");
            }
            ...
            LogCommentWarning(CL, "Wait for maintenance to start after 10 mins in hot standby.");
            int defaultMaintenanceDelayInt1 = Convert.ToInt32(defaultMaintenanceDelay);
            defaultMaintenanceDelayInt1 = defaultMaintenanceDelayInt1 * 60;

            // Fetch the maintenaceStart milestone from milestones.ini
            String maintenanceStartMilestone1 = CL.EA.UI.Utils.GetValueFromMilestones("MaintenanceStart");

            //  Begin wait for maintenaceStart milestone
            CL.EA.UI.Utils.BeginWaitForDebugMessages(maintenanceStartMilestone1, defaultMaintenanceDelayInt1 + 180);

            System.Collections.ArrayList arrayList1 = new System.Collections.ArrayList();
            bool isMaintenanceMilestoneRecieved1 = CL.EA.UI.Utils.EndWaitForDebugMessages(maintenanceStartMilestone1, ref arrayList1);
            if (!isMaintenanceMilestoneRecieved1)
            {
                FailStep(CL, res, "Failed to start maintenance ");
            }
            ...
            //  Fetch the maintenaceComplete milestone from milestones.ini
            String maintenanceCompleteMilestone1 = CL.EA.UI.Utils.GetValueFromMilestones("MaintenanceComplete");

            int MaintenanceDurationInt1 = Convert.ToInt32(MaintenanceDuration) * 60;

            //  Begin wait for maintenaceComplete milestone
            CL.EA.UI.Utils.BeginWaitForDebugMessages(maintenanceCompleteMilestone1, MaintenanceDurationInt1);


            bool maintenanceCompleteMilestoneRecieved1 = CL.EA.UI.Utils.EndWaitForDebugMessages(maintenanceCompleteMilestone1, ref arrayList1);
            if (!maintenanceCompleteMilestoneRecieved1)
            {
                FailStep(CL, res, "Failed to complete the maintenance");
            }
            PassStep();
