/// <summary>
///  Script Name : TM_StartPlaybackPerformance.cs
///  Test Name   : TM_StartPlaybackPerformance
///  TEST ID     : 
///  QC Version  : 
///  Variations from QC:
/// ----------------------------------------------- 
...
[Test("TM_StartPlaybackPerformance")]
public class TM_StartPlaybackPerformance : _Test
{
    [ThreadStatic]
    static _Platform CL;
    ...
    static string playOption;
    static string playfromBeginningOption;
    ...
    private class PreCondition : _Step
    {
        public override void Execute()
        {
            ...
            context = CL.EA.GetValueFromINI(EnumINIFile.Test, "TEST PARAMS", "CONTEXT");
            if (string.IsNullOrEmpty(context))
            {
                
                FailStep (CL, "Failed to fetch CONTEXT  items in TEST PARAMS from the Test ini file!");
            }

            playOption = CL.EA.GetValueFromINI(EnumINIFile.Test, "TEST PARAMS", "PLAYOPTION");
            if (string.IsNullOrEmpty(playOption))
            {

                FailStep(CL, "Failed to fetch PLAYOPTION  items in TEST PARAMS from the Test ini file!");
            }

            playfromBeginningOption = CL.EA.GetValueFromINI(EnumINIFile.Test, "TEST PARAMS", "PLAYFROMBEGINNINGOPTION");
            if (string.IsNullOrEmpty(playfromBeginningOption))
            {

                FailStep(CL, "Failed to fetch PLAYFROMBEGINNINGOPTION  items in TEST PARAMS from the Test ini file!");
            }
    ...
    private class Step1 : _Step
    {
        ...
        public override void Execute()
        {
            ...
            for (int i = 1; i <= numberOfIteration; i++)
            {
                ...
                LogComment(CL, "Navigate to archive Archive for StartPlayback.Iteration : " + i.ToString());


                string timeStamp = "";
                string OptionToCheck = "";
               
                //res=CL.IEX.MilestonesEPG.Navigate(context);

                if ((CL.EA.UI.Utils.EPG_Milestones_Navigate(context)).Equals(false))
                {
                    FailStep(CL, res, "Failed to navigate to "+context+" in Archive ");
                }


                if (i == 1)
                {
                    OptionToCheck = playOption; 
                }
                else
                {
                    OptionToCheck = playfromBeginningOption; 
                }




                if ((CL.EA.UI.Utils.EPG_Milestones_SelectMenuItem(OptionToCheck)).Equals(false))
                {
                    FailStep(CL, "Failed to highlight " + OptionToCheck + " Option ");
                }
            ...
            double averageTime = temp / numberOfIteration;
