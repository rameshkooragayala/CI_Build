/// <summary>
///  Script Name        : LIB_1002_Termination_AdultSessionOpened.cs
///  Test Name          : LIB-1000-Valid entry of PIN code in Library for adult session
///  TEST ID            : 23603
///  QC Version         : 2
...
public class LIB_1000 : _Test
{
    ...
    public override void CreateStructure()
    {
        ...
        this.AddStep(new Step2(), "Step 2: Navigate to Planner, Enter Pin and verify Proper Event name is displayed");
    ...
    private class PreCondition : _Step
    {
        public override void Execute()
        {
            ...
            //Book future events from Guide for verification in planner
            res = CL.EA.SurfToChannelInGuide(lockedService.LCN);
            if (!res.CommandSucceeded)
            {
                FailStep(CL,res,"Failed to tune to service "+lockedService.LCN);
            }
            string timestamp="";
            ...
            //Enter PIN
            CL.EA.UI.Utils.EnterPin(defaultPIN);
            res=CL.EA.PVR.BookFutureEventFromGuide("FutureEventBasedRecording1",lockedService.LCN,NumberOfPresses:3,VerifyBookingInPCAT:false,ReturnToLive:false);
            if(!res.CommandSucceeded)
            {
                FailStep(CL,res,"Failed to book future event from Guide");
            }

            res=CL.EA.PVR.BookFutureEventFromGuide("FutureEventBasedRecording2",lockedService.LCN,NumberOfPresses:1,VerifyBookingInPCAT:false);
            if(!res.CommandSucceeded)
            {
                FailStep(CL,res,"Failed to book future event from Guide");
            }
    ...
    private class Step2 : _Step
    {
        public override void Execute()
        {
            StartStep();
            //Navigate to 'My recordings'
            res = CL.EA.PVR.NavigateToPlanner();
            if (!res.CommandSucceeded)
            {
                FailStep(CL, res, "Could not navigate to 'My recordings'");
            }
